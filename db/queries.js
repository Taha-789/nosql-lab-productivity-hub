...
async function listUserProjects(db, ownerId) {
  arr=await db.collection('projects').find({ownerId: ownerId}).sort({createdAt: -1}).toArray()
  return arr
}
...
async function createProject(db, projectData) {
  const result=await db.collection('projects').insertOne({ownerId:projectData.ownerId,name:projectData.name,description:projectData.description,archived:false,createdAt:new Date()})
...
async function archiveProject(db, projectId) {
  const result=await db.collection('projects').updateOne({_id:projectId},{$set:{archived:true}})
   return {
